...
import { useFetchLeaderBoardQuery } from "../../query/api/user/leaderboard";

export const Leaderboard = () => {
  const { id } = useParams();
  const { data: leaderboard, isLoading } = useFetchLeaderBoardQuery(id || "");
  const totalEnrolledUsers = useMemo(() => {
    return leaderboard?.data.totalEnrolledUsers;
  }, [leaderboard]);
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-4 pr-2 overflow-y-scroll max-h-[calc(100vh-280px)]">
            {leaderboard?.data.leaderboard.map((user, index) => (
              <div
                key={user._id}
                className="p-3 bg-[#efede5] hover:bg-gray-50 transition-colors rounded-lg border border-gray-100"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    {/* Rank */}
                    <div
                      className={`flex items-center justify-center w-8 h-8 rounded-lg border ${getRankBadgeStyle(
                        index
                      )}`}
                    >
