...
import { useFetchEnrolledChallengesQuery } from "../../query/api/user/challanges";
...
export const UserChallanges = () => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useFetchEnrolledChallengesQuery(id || "");
  const isCompleted = useMemo(() => {
    return enrolledChallenges?.isCompleted;
  }, [enrolledChallenges?.isCompleted]);
  const userChallenges = useMemo(() => {
    return enrolledChallenges?.data;
  ...
  if (!userChallenges && !isLoading) return;
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 place-content-start gap-4 pr-2 overflow-y-scroll h-[95%]">
            {userChallenges?.map((challenge) => (
              <div
                key={challenge._id}
                className={`grid grid-rows-1 grid-cols-9 place-items-center justify-items-start rounded-xl p-4 transition-shadow ${
              ...
              >
                {!isCompleted && (
                  <Link
                    to={`/user/quest/${id}/progress/video/feed/${challenge._id}`}
                    className="contents"
                  >
                    <div className="col-span-1 flex items-center">
                      <Flag
                        className={`${
                          challenge.isCompleted
                            ? "text-green-400 bg-green-100 rounded-full p-2 h-fit w-fit"
                            : "text-gray-400 bg-gray-200 rounded-full p-2 h-fit w-fit"
                        }`}
                      />
                    </div>
                    <div className="col-span-2 text-xs md:text-sm font-medium text-black">
