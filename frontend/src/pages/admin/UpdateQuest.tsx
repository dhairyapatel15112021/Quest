...
export const UpdateQuest = () => {
  ...
  const { mutate: updateQuest, isPending } = useQuestMutation();
    ...
    if (currentQuest) {
      setQuestData({
        ...
        end_date: currentQuest.end_date
          ? new Date(currentQuest.end_date)
          : new Date(),
      });
  ...
  const handleSubmitFunction = async () => {
    ...
    if (
      !alwaysOn &&
      new Date(questData.end_date) < new Date(questData.start_date)
    ) {
    ...
    console.log(id);
    updateQuest(
      { data: questData, alwaysOn, isQuestCreation: false },
      {
            ...
            return prevQuests.map((q) =>
              q.id === id
                ? {
                    ...response.data.quest,
                    ...q,
                    image_url: `data:image/png;base64,${response.data.quest.quest_image}`,
                  }
                : q
            );
          });
        ...
        <fieldset className="fieldset w-full">
          ...
          <div className="flex justify-between items-center">
            <div
              ...
              className={`flex justify-center items-center text-black w-[50%] transition duration-200 p-2 rounded-md cursor-pointer ${
