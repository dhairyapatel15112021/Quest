...
export const Participate: React.FC = () => {
  ...
  const { data: participantsData, isLoading } = useFetchParticipantsQuery(
    id || ""
  );
        ...
        <div className="flex items-center space-x-4">
          <div className="bg-amber-50 text-amber-800 py-2 px-4 rounded-lg border border-amber-200">
            <span className="font-medium">
              Total Participants:{" "}
              {participantsData?.data.totalParticipants || 0}
            </span>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 overflow-y-scroll h-full">
            {filteredParticipants.map((participant, index) => (
              <div
                key={participant._id}
                className="rounded-2xl shadow-sm bg-[#efede5] border border-gray-100 p-3 px-6"
              >
                  ...
                  <div className="col-span-3 flex items-center space-x-6">
                    <div className="text-center">
                      <p className="text-2xl font-bold text-amber-600">
                        {participant.rewards.points || 0}
                      </p>
                      <p className="text-sm text-gray-500">Points</p>
                    </div>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-amber-600">
                        {participant.rewards.freeCoffee || 0}
                      </p>
          ...
          <div className="w-full mt-auto h-full flex items-center justify-center">
            <h1 className="text-2xl mt-[10%] font-medium text-gray-600">
              No Participants
            </h1>
          </div>
        )}
