...
import { DashboardData, useQuestsQuery } from "../../query/api/admin/dashboard";

export const Dashboard = () => {
  const [quests, setQuests] = useState<DashboardData["quests"]>([]);
  const [overallStats, setOverallStats] =
    useState<DashboardData["overallStats"]>();
  const setQuestIdAtom = useSetRecoilState(questIdAtom);
  const setQuestAtom = useSetRecoilState(questAtom);
  const navigate = useNavigate();
  const { data: questsData, isLoading, isError } = useQuestsQuery();

  const data = [
    {
      title: "Number of Active Quests",
      key: "totalActiveQuests" as keyof DashboardData["overallStats"],
    },
    {
      title: "Number of Liked Videos",
      key: "totalLikedVideos" as keyof DashboardData["overallStats"],
    },
    {
      title: "Number of Shared Videos",
      key: "totalSharedVideos" as keyof DashboardData["overallStats"],
    },
    {
      title: "Total Points Distributed",
      key: "totalRewardsDistributed" as keyof DashboardData["overallStats"],
    },
  ];

  useEffect(() => {
    if (!questsData) return;
    const processedQuests: DashboardData["quests"] = questsData.quests.map(
      (quest) => ({
        ...quest,
        quest_image: quest.quest_image
          ? `data:image/jpeg;base64,${quest.quest_image}`
          : "",
      })
    );
    ...
    setOverallStats(questsData.overallStats);
  }, [questsData]);

  useEffect(() => {
    if (isError) {
      toast.error("An error occurred while fetching quests");
    }
  }, [isError]);
      ...
      <div className="w-full h-[73vh] flex flex-col mt-[2vh] px-10 py-2">
        ...
        <div className="sticky top-0 z-10 bg-[#f8f7f3] pb-4">
          <div className="flex justify-between items-center">
            <div className="text-black text-xl font-medium md:font-semibold">
              Quests
            </div>
            <Link
          ...
          {isLoading ? (
            ...
            <div className="space-y-4 pt-2">
              {quests.map((quest) => (
                <div
                  key={quest._id}
                  className="grid grid-cols-9 grid-rows-1 place-items-start text-gray-500 py-2 font-bold text-sm"
                >
                  <div className="col-span-6 grid grid-cols-4 grid-rows-1 w-full">
                    <div className="flex justify-center items-center">
                      {" "}
                      <img
                        onClick={() => redirectToQuestDetails(quest._id)}
                        src={quest.quest_image}
                        className="rounded-2xl h-[100px] cursor-pointer"
                        alt="quest.png"
                      />
                    </div>
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="p-1 text-xs font-extralight rounded-md text-gray-600 border-[0.2px] border-gray-400 w-fit">
                        {quest.total_budget}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-start item-center p-1 h-fit text-xs font-extralight rounded-md text-gray-600 border-[0.2px] border-gray-400 w-fit">
                    {quest.is_Active === "created"
                      ? "Unpublished"
                      : quest.is_Active === "completed"
                      ? "Completed"
                      : "Published"}
                  </div>
                  <div
