...
export const QuestBanner = () => {
  ...
  const { mutate: toggleQuest, isPending } = useQuestToggleMutation();
  ...
  const handleToggle = async (value: boolean) => {
    toggleQuest(questData?.id || "", {
      onSuccess: (response) => {
        setQuestData(() => ({
          ...questData,
          Title: questData?.Title || "",
          Description: questData?.Description || "",
          start_date: questData?.start_date || new Date(),
          end_date: questData?.end_date || new Date(),
          total_budget: questData?.total_budget || 0,
          total_participates: questData?.total_participates || 0,
          quest_image: questData?.quest_image || new Uint8Array(),
          id: questData?.id || "",
      ...
      {isLoading ? (
        ...
        <div className="h-[30vh] bg-gray-700 text-white rounded-2xl px-28 flex flex-col justify-between">
            ...
            <div className="flex justify-center items-center gap-2">
              ...
              <div className="dropdown dropdown-end">
                <div
                  tabIndex={0}
                  role="button"
                  className="cursor-pointer rounded-full p-2 bg-gray-500 text-white"
                >
                  <EllipsisHorizontal />
                </div>
                <ul
                ...
                >
                  <li>
                    <div className="flex justify-between items-center gap-2 w-full h-full">
                      <div>
                        {questData?.is_Active === "created"
                          ? "Unpublished"
                          : questData?.is_Active === "completed"
                          ? "Completed"
                          : "Published"}
                      </div>
                      <div>
                        <input
                          type="checkbox"
                          className={`toggle h-6 w-9 bg-none transition-all duration-300 ${
                            questData?.is_Active === "created"
                              ? "!bg-red-200"
                              : "!bg-green-200"
                          }`}
          ...
          <div className="px-2 flex justify-between items-center">
            <div className="flex flex-col items-start gap-1">
              ...
              <div className="font-light text-base">
                {questData?.start_date.toString().split("T")[0]}
                {questData?.end_date
                  ? ` - ${questData?.end_date.toString().split("T")[0]}`
                  : " - ♾️"}
              </div>
              <div className="font-extralight bg-gray-800 px-2 py-1 rounded-md w-fit">
                {questData?.total_budget}
              </div>
            </div>
            <img
              src={questData?.image_url}
              alt="photo.png"
          ...
          <div className="flex gap-4">
            ...
            <NavLink
              to={`/admin/quest/${id}/participates`}
              className={({ isActive }) =>
                `${
                  isActive ? "border-b-[3px] border-orange-300" : "border-none"
                } text-base py-3`
