...
export const QuestCreate = () => {
  ...
  const { mutate: createQuest, isPending } = useQuestMutation();
  ...
  const handleSubmitFunction = async () => {
    ...
    if (questData.Title.trim().length < 3) {
    ...
    if (questData.Description.trim().length < 10) {
    ...
    // Budget validation
    if (
      isNaN(Number(questData.total_budget)) ||
      Number(questData.total_budget) <= 0
    ) {
    ...
    // Image validation
    if (!questData.quest_image || questData.quest_image.length === 0) {
    ...
    // Date validation
    const now = new Date();
    const startDate = new Date(questData.start_date);
    ...
    createQuest(
      { data: questData, alwaysOn, isQuestCreation: true },
      {
  ...
  return (
    <div className="h-[90vh] text-black px-2 md:px-10 py-2 flex justify-between items-start">
      ...
      <div>
        <div className="font-medium text-base md:text-lg text-center">
          New Quest
        </div>
        <div className="w-[70vw] md:w-[25vw] h-[85vh] mt-[2vh]">
          <div className="rounded-md h-[20vh] w-full flex justify-center items-center">
            ...
            <label
              htmlFor="fileInput"
              className={`${
                imagePreview ? "bg-none" : "bg-[#ecd1b2] "
              } transition duration-200 flex flex-col gap-2 justify-center items-center rounded-md w-full h-full cursor-pointer relative}`}
            >
              {imagePreview ? (
          ...
          <fieldset className="fieldset w-full">
            ...
            <div className="flex justify-between items-center">
              <div
                ...
                className={`flex justify-center items-center w-[50%] transition duration-200 p-2 rounded-md cursor-pointer ${
                  !alwaysOn ? "font-medium bg-[#ecd1b2]" : "font-base bg-none"
                }`}
              >
                Fixed Dates
              </div>
      ...
      <button
        ...
        className="group bg-blue-500 text-white text-xs md:text-base px-2 md:px-4 py-1 rounded-3xl border border-blue-500 hover:bg-white hover:text-blue-500 cursor-pointer"
      >
        {isPending ? (
