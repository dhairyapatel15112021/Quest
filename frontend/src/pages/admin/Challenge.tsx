...
} from "../../query/api/admin/challenge";
...
export const Challenge = () => {
  ...
  const { mutate: createChallengeMutation, isPending } =
    useCreateChallengeMutation();

  useEffect(() => {
    setLoading(true);
    const quest: QuestState | null =
      quests?.filter((q) => q.id === id)[0] || null;
    if (quest == null) {
      setLoading(false);
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!likeState && !shareState) {
      errors.push("At least one of Like or Share must be selected.");
    }
    if (likeState && challange.like_video_count <= 0) {
      errors.push("Like count must be greater than 0.");
    }
    ...
    createChallengeMutation(
      { challenge: challange, reward: reward },
      {
  ...
  return (
    <>
      {isLoading ? (
        ...
        <div className="h-[90vh] text-black px-2 md:px-10 py-2 flex justify-between items-start shrink-0">
          <Link
            to={`/admin/quest/${id}/overview`}
            ...
            <div className="w-[70vw] md:w-[25vw] h-[85vh] mt-[2vh]">
                ...
                <div className="flex gap-2">
                  ...
                  <div className="flex flex-col gap-2 w-full justify-between items-center">
                    <div
                      ...
                      onClick={() => setRewardCouponState(!rewardCouponState)}
                    >
                      Coupons?
                    </div>
                    {rewardCouponState && (
              ...
              <fieldset className="fieldset w-full mt-2">
                <div className="flex justify-between items-center">
                  <legend className="fieldset-legend text-base text-gray-700">
                    Reward Expiry
                  </legend>
                  <div
                    className="tooltip tooltip-bottom md:tooltip-right flex justify-between items-center"
                    data-tip="How many days reward should active after claim."
                  >
                    <button>
                      <Info />
                    </button>
                ...
                <input
                  onChange={(event) =>
                    setExpiryDuration(parseInt(event.target.value))
                  }
          ...
          <button
            ...
            className="group bg-blue-500 text-white text-xs md:text-base px-2 md:px-4 py-1 rounded-3xl border border-blue-500 hover:bg-white hover:text-blue-500 cursor-pointer min-w-24"
          >
            {isPending ? (
              <Loader className="text-white group-hover:bg-blue-500" />
            ) : (
              "Create"
            )}
          </button>
